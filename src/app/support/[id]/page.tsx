...
export default function TicketDetailPage() {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 16 }}>
                    ...
                    <PermissionGate module="support" action="edit">
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            {ticket.status !== 'resolved' && <button onClick={() => handleStatusChange('resolved')} style={actionBtn('var(--color-success)')}><CheckCircle size={14} /> {t('support.resolve')}</button>}
                            {ticket.status !== 'closed' && <button onClick={() => handleStatusChange('closed')} style={actionBtn('var(--text-secondary)')}><XCircle size={14} /> {t('support.close')}</button>}
                            <button onClick={() => { setReassignTo(ticket.assigned_to || ''); setShowReassign(true); }} style={actionBtn('var(--color-info)')}><UserCheck size={14} /> {t('support.reassign')}</button>
                        </div>
                    </PermissionGate>
                </div>
            ...
            {/* Message Thread */}
            <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: 12, padding: 24 }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 600, margin: '0 0 16px' }}>{t('support.conversation')} ({ticket.messages.length})</h3>
            ...
            <FormModal
                ...
                title={`${t('support.reassignTicket')} — ${ticket.id}`}
            ...
            >
                <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 8, fontSize: '0.875rem' }}>
                    <strong>{t('support.currentlyAssignedTo')}:</strong> {ticket.assigned_to_name || t('support.unassigned')}
                </div>
