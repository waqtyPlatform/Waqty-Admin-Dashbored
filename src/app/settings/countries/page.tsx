...
export default function CountriesPage() {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <DataTable<Country> columns={columns} data={mockCountries} searchKeys={['name', 'name_ar', 'code']} searchPlaceholder="Search countries..." getRowKey={r => r.id} />

            <FormModal open={showCreate} onClose={() => setShowCreate(false)} title="Add Country" submitLabel="Add Country" onSubmit={e => { e.preventDefault(); setShowCreate(false); }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <FormField label="Name (English)" required><input type="text" required style={inputStyle} placeholder="Egypt" /></FormField>
                    <FormField label="Name (Arabic)" required><input type="text" required style={inputStyle} placeholder="مصر" dir="rtl" /></FormField>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <FormField label="Country Code" required><input type="text" required style={inputStyle} placeholder="EG" maxLength={3} /></FormField>
                    <FormField label="Currency" required><input type="text" required style={inputStyle} placeholder="EGP" maxLength={3} /></FormField>
                </div>
            </FormModal>
        </div>
    );
