...
export default function ReviewAnalyticsPage() {
    ...
    const statusDist = [
        { name: t('reviews.analytics.published'), count: mockReviews.filter(r => r.status === 'published').length, color: 'var(--color-success)' },
        { name: t('reviews.analytics.flagged'), count: mockReviews.filter(r => r.status === 'flagged').length, color: 'var(--color-warning)' },
        { name: t('reviews.analytics.hidden'), count: mockReviews.filter(r => r.status === 'hidden').length, color: 'var(--color-error)' },
        { name: t('reviews.analytics.pending'), count: mockReviews.filter(r => r.status === 'pending').length, color: 'var(--color-info)' },
    ];
    return (
        <div className={shared.page}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
                {[
                    { label: t('reviews.analytics.totalReviews'), value: mockReviews.length, icon: <MessageSquare size={20} />, color: 'var(--color-primary-500)' },
                    { label: t('reviews.analytics.avgRating'), value: avgRating, icon: <Star size={20} />, color: '#f59e0b' },
                    { label: t('reviews.analytics.flagged'), value: mockReviews.filter(r => r.status === 'flagged').length, icon: <AlertTriangle size={20} />, color: 'var(--color-warning)' },
                    { label: t('reviews.analytics.responseRate'), value: `${Math.round(mockReviews.filter(r => r.admin_response).length / mockReviews.length * 100)}%`, icon: <TrendingUp size={20} />, color: 'var(--color-success)' },
                ].map(k => (
            ...
            <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: 12, padding: 20 }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead><tr style={{ background: 'var(--bg-secondary)' }}>
                        {[t('reviews.analytics.provider'), t('reviews.analytics.reviews'), t('reviews.analytics.avgRating')].map(h => <th key={h} style={{ textAlign: 'left', padding: '10px 16px', color: 'var(--text-secondary)', fontWeight: 500, fontSize: '0.75rem', textTransform: 'uppercase', borderBottom: '1px solid var(--border-color)' }}>{h}</th>)}
                    </tr></thead>
